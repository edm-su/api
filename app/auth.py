...
async def get_current_user(token: str = Depends(oauth_scheme)) -> typing.Mapping:
...
async def get_current_user_or_guest(token: str = Depends(oauth_scheme)) -> typing.Optional[typing.Mapping]:
    if token:
        db_user = await get_current_user(token=token)
        return db_user


async def get_current_admin(db_user: dict = Depends(get_current_user)) -> typing.Mapping:
    if not db_user['is_admin']:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Пользователь не является администратором',
                            headers={'WWW-Authenticate': 'Bearer'})
    return db_user
