...
def get_videos_count(db: Session, parent=None):
    if parent:
        ...
        videos = db.query(models.Video).filter_by(deleted=False).count()
    return videos
...
def get_videos(db: Session, skip: int = 0, limit: int = 25, parent=None, deleted: bool = False):
    if parent:
        ...
        videos = db.query(models.Video).filter_by(deleted=deleted).order_by(desc('date')).offset(skip).limit(
            limit).all()
    return videos


def get_related_videos(title: str, db: Session, limit: int = 25):
    videos = db.query(models.Video).filter_by(deleted=False).order_by(models.Video.title.op('<->')(title)).limit(
        limit).offset(1).all()
    return videos


def get_video_by_slug(db: Session, slug: str, deleted: bool = False):
    return db.query(models.Video).filter_by(slug=slug).filter_by(deleted=deleted).first()


def delete_video(db: Session, video: models.Video):
    video.deleted = True
    db.add(video)
    db.commit()
    db.refresh(video)
